...
const convertLargeToWord = number => {
  ...
  const groups = groupByThreeDigits(number)
  const word = groups.reduce((prevResult, numberGroup, index) => {
    ...
    do {
      nextIndex += 1
      remainingValue = remainingValue * 1000 + groups[nextIndex]
    } while (nextIndex < groups.length - 1)
